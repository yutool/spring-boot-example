...
public class JwtAuthorizationTokenFilter extends BasicAuthenticationFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
        String authToken  = request.getHeader(JwtTokenUtils.TOKEN_HEADER);

        // 如果请求头中没有Authorization信息则直接放行了
        if (authToken == null || !authToken.startsWith(JwtTokenUtils.TOKEN_PREFIX)) {
    ...
    private UsernamePasswordAuthenticationToken getAuthentication(String authToken) {
        String token = authToken.replace(JwtTokenUtils.TOKEN_PREFIX, "");
