...
import com.example.oauth.auth.MyAccessDeniedHandler;
import com.example.oauth.auth.MyFilterSecurityInterceptor;
import com.example.oauth.jwt.*;
import com.example.oauth.jwt.JwtAuthenticationEntryPoint;
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(AuthenticationManagerBuilder auth) throws Exception {
        ...
        auth.inMemoryAuthentication()
                .withUser("zs").password(bCryptPasswordEncoder().encode("123456")).roles("USER")
                .and().withUser("ls").password(bCryptPasswordEncoder().encode("123456")).roles("ADMIN");
...
//        auth.userDetailsService(userDetailsService).passwordEncoder(bCryptPasswordEncoder());
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.cors().and().csrf().disable()
            ...
            .and().apply(smsSecurityConfig)
            // 过滤请求
            .and().authorizeRequests()
                // 不需要认证的接口
                .antMatchers("/api/login").permitAll()
                // 手动配置需要权限的接口，配置了动态的这里就不起效果了
